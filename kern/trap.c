...
trap_init(void)
{
	...
	SETGATE(idt[T_DEFAULT], 0, GD_KT, handler500, 0);
...
trap_init_percpu(void)
{
	...
	gdt[(GD_TSS0 >> 3)+cpu_i] = SEG16(STS_T32A, (uint32_t) (ts), sizeof(struct Taskstate), 0);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 4: Your code here.
//	cprintf("handling env_pgfault_upcall=%d\n", curenv->env_pgfault_upcall);
	if (curenv->env_pgfault_upcall){
		struct UTrapframe *p_utf;
		p_utf=(struct UTrapframe*)(((UXSTACKTOP-PGSIZE<=tf->tf_esp && tf->tf_esp<UXSTACKTOP)?(tf->tf_esp-4):UXSTACKTOP)-sizeof(struct UTrapframe));
		user_mem_assert(curenv, (void*)p_utf, sizeof(struct UTrapframe), PTE_W);
